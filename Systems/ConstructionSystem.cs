    ...
    public static class ConstructionSystem
    {
        public static bool TryBuild(Agent agent, Tile tile, Random rng)
        {
            ...
            foreach (var ax in EffectTables.Axis.Keys)
            {
                ...
                profile[ax] = v;
                if (v > dominantValue) { dominantValue = v; dominant = ax; }
            }
            ...
            tile.BuildingName = $"{Root(best[0].spec)}{Root(best[1].spec)}-{EffectTables.AxisBuildingWord(dominant)}";
